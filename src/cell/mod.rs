...
impl CellData {
    ...
    pub fn with_params<TRefs>(refs: TRefs, data: Vec<u8>, cell_type: CellType, level_mask: u8, 
        hashes: Option<Vec<UInt256>>, depths: Option<Vec<u16>>) -> Result<CellData>  
    where
        TRefs: IntoIterator<Item = Arc<CellData>>
    {
        ...
        let mut references: Vec<Arc<CellData>> = vec![];
    ...
    pub fn reference(&self, index: usize) -> Result<&Arc<CellData>> {
        self.references.get(index).ok_or(ExceptionCode::CellUnderflow)
    }
    ...
    pub fn calc_bit_length(&self) -> usize {
        find_tag(&self.data)
    }
    ...
    pub fn format_without_refs(&self, f: &mut fmt::Formatter, indent: &str, last_child: bool, 
        ...
        write!(f, "bits: {}", self.calc_bit_length())?;
        write!(f, "   refs: {}", self.references.len())?;
        ...
        if full {
            if let (Some(hashes), Some(depths)) = (self.hashes.as_ref(), self.depths.as_ref()) {
                write!(f, "\n")?;
                if !root { Self::print_indent(f, indent, last_child, false)?; }
                write!(f, "hashes:")?;
                ...
                write!(f, "\n")?;
                if !root { Self::print_indent(f, indent, last_child, false)?; }
                write!(f, "hashes, depths -not finalized-")?;
            }
        }
        Ok(())
    }
    ...
    pub fn format_with_refs_tree(
        ...
        if remaining_depth > 0 {
            ...
            for (i, child) in self.references().iter().enumerate() {
                write!(f, "\n")?;
                indent = child.format_with_refs_tree(
                    f, indent, i == self.references().len() - 1, full, false, remaining_depth - 1)?;
            }
            if !root {
                indent.pop();
                indent.pop();
...
impl PartialEq for CellData {
    fn eq(&self, other: &CellData) -> bool {
        debug_assert!(self.hashes.is_some() && other.hashes.is_some());
...
impl fmt::LowerHex for CellData {
    fn fmt(&self, f: &mut fmt::Formatter) -> fmt::Result {
        write!(f, "{}",  self.to_hex_string(true))
    }
}

impl fmt::UpperHex for CellData {
    fn fmt(&self, f: &mut fmt::Formatter) -> fmt::Result {
        write!(f, "{}",  self.to_hex_string(false))
    }
}

impl fmt::Binary for CellData {
    fn fmt(&self, f: &mut fmt::Formatter) -> fmt::Result {
