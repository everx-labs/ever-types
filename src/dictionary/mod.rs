    ...
    where F: FnMut(SliceData, Option<SliceData>, Option<SliceData>) -> Result<bool> {
        ...
        if l1 > l2 && cursor_self != SliceData::default() && cursor_other != SliceData::default(){
            ...
            while queue.len() > ind {
                if queue[ind].2 <= time_l {
                    if queue[ind].1 == key1 {
                        dict_scan_diff(&mut cursor1, &mut queue[ind].3.clone(), key1.clone(), queue[ind].4.clone(), time_l, queue[ind].5.clone(), &mut op)?;
                    }
                    else {
                        dict_scan_diff(&mut SliceData::default(), &mut queue[ind].3.clone(), key1.clone(), queue[ind].4.clone(), time_l, queue[ind].5.clone(), &mut op)?;
                    }
                }
                else {
        ...
        if l1 < l2 && cursor_self != SliceData::default() && cursor_other != SliceData::default(){
            ...
            while queue.len() > ind {
                if queue[ind].2 <= time_l {
                    if queue[ind].1 == key2 {
                        dict_scan_diff(&mut queue[ind].3.clone(), &mut cursor2, queue[ind].4.clone(), key2.clone(), queue[ind].5.clone(), time_l, &mut op)?;
                    }
                    else {
                        dict_scan_diff(&mut queue[ind].3.clone(), &mut SliceData::default(), queue[ind].4.clone(), key2.clone(), queue[ind].5.clone(), time_l, &mut op)?;
                    }
                }
                else {
                    let n = cmp::min(2, queue[ind].0.remaining_references());
        ...
        dict_scan_diff(&mut cursor_self, &mut cursor_other, BuilderData::default(), BuilderData::default(), bit_len_self, bit_len_other, &mut op)?;
        Ok(true)
    }
}
...
where F: FnMut(SliceData, Option<SliceData>, Option<SliceData>) -> Result<bool> {
    ...
    if cursor_1.cell() == cursor_2.cell() {
        return Ok(true);  
    }
    ...
    if bit_len_1 == l1 && bit_len_2 != l2 {
        ...
        if chk == 0 { 
            op(fkey, Some(cursor_1.clone()), None)?; 
        }
        return Ok(true);
    }
    if bit_len_1 != l1 && bit_len_2 == l2 {
        ...
        if chk == 0 { 
            op(fkey, None, Some(cursor_2.clone()))?; 
        }
        return Ok(true);
    }
    if bit_len_1 - l1 == bit_len_2 - l2{
        ...
        assert!(key1 == key2);
        if bit_len_1 == l1 {
            op(key1.into(), Some(cursor_1.clone()), Some(cursor_2.clone()))?;
            return Ok(true);
        }
        ...
        for i in 0..n {
            ...
            dict_scan_diff(child1, child2, key.clone(), key.clone(), bit_len_1 - l1 - 1, bit_len_2 - l2 - 1, op)?;
        } 
        for i in n..n2 {
            let mut key = key1.clone();
            key.append_bit_bool(i != 0)?;
            let ref mut child = SliceData::from(cursor_1.reference(i)?);
            dict_scan_diff(child, &mut SliceData::default(), key.clone(), key.clone(), bit_len_1 - l1 - 1, bit_len_2, op)?;
        }  
        for i in n..n1 {
            let mut key = key1.clone();
            key.append_bit_bool(i != 0)?;
            let ref mut child = SliceData::from(cursor_2.reference(i)?);
            dict_scan_diff(&mut SliceData::default(), child, key.clone(), key.clone(), bit_len_1, bit_len_2 - l2 - 1, op)?;
        }   
        return Ok(true);
    }
    return Ok(true);
