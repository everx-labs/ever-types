    ...
    where F: FnMut(SliceData, Option<SliceData>, Option<SliceData>) -> Result<bool> {
        ...
        if l1 > l2 && cursor_self != SliceData::default() && cursor_other != SliceData::default(){
            ...
            while queue.len() > ind {
                if queue[ind].2 <= time_l {
                    if queue[ind].1 == key1 {
                        let res = dict_scan_diff(&mut cursor1, &mut queue[ind].3.clone(), key1.clone(), queue[ind].4.clone(), time_l, queue[ind].5.clone(), &mut op);
                        if let Err(e) = res {
                            return Err(e);
                        }
                    }
                    else {
                        let res = dict_scan_diff(&mut SliceData::default(), &mut queue[ind].3.clone(), key1.clone(), queue[ind].4.clone(), time_l, queue[ind].5.clone(), &mut op);
                        if let Err(e) = res {
                            return Err(e);
                        }
                    }
        ...
        if l1 < l2 && cursor_self != SliceData::default() && cursor_other != SliceData::default(){
            ...
            while queue.len() > ind {
                if queue[ind].2 <= time_l {
                    if queue[ind].1 == key2 {
                        let res = dict_scan_diff(&mut queue[ind].3.clone(), &mut cursor2, queue[ind].4.clone(), key2.clone(), queue[ind].5.clone(), time_l, &mut op);
                        if let Err(e) = res {
                            return Err(e);
                        }
                    }
                    else {
                        let res = dict_scan_diff(&mut queue[ind].3.clone(), &mut SliceData::default(), queue[ind].4.clone(), key2.clone(), queue[ind].5.clone(), time_l, &mut op);
                        if let Err(e) = res {
                            return Err(e);
                        }
                    }
        ...
        let res = dict_scan_diff(&mut cursor_self, &mut cursor_other, BuilderData::default(), BuilderData::default(), bit_len_self, bit_len_other, &mut op);
        if let Err(e) = res {
            return Err(e);
        }
        Ok(true)
...
where F: FnMut(SliceData, Option<SliceData>, Option<SliceData>) -> Result<bool> {
    ...
    if bit_len_1 == l1 && bit_len_2 != l2 {
        ...
        if chk == 0 { 
            let res = op(fkey, Some(cursor_1.clone()), None); 
            if let Err(e) = res {
                return Err(e);
            }
        }
    ...
    if bit_len_1 != l1 && bit_len_2 == l2 {
        ...
        if chk == 0 { 
            let res = op(fkey, None, Some(cursor_2.clone())); 
            if let Err(e) = res {
                return Err(e);
            }
        }
    ...
    if bit_len_1 - l1 == bit_len_2 - l2{
        ...
        assert!(key1 == key2);
        if bit_len_1 == l1 {
            let res = op(key1.into(), Some(cursor_1.clone()), Some(cursor_2.clone()));
            if let Err(e) = res {
                return Err(e);
            }
            return Ok(true);
        ...
        for i in 0..n {
            ...
            let res = dict_scan_diff(child1, child2, key.clone(), key.clone(), bit_len_1 - l1 - 1, bit_len_2 - l2 - 1, op);
            if let Err(e) = res {
                return Err(e);
            }
        } 
        for i in n..n2 {
            ...
            let res = dict_scan_diff(child, &mut SliceData::default(), key.clone(), key.clone(), bit_len_1 - l1 - 1, bit_len_2, op);
            if let Err(e) = res {
                return Err(e);
            }
        }  
        for i in n..n1 {
            ...
            let res = dict_scan_diff(&mut SliceData::default(), child, key.clone(), key.clone(), bit_len_1, bit_len_2 - l2 - 1, op);
            if let Err(e) = res {
                return Err(e);
            }
        }   
