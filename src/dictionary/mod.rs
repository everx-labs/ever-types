...
pub trait HashmapType: Sized {
    ...
    where F: FnMut(SliceData, Option<SliceData>, Option<SliceData>) -> Result<bool> {
        ...
        let mut cursor1 = cursor_self.clone();
        let mut cursor2 = cursor_other.clone();
        let label1 = cursor1.get_label(bit_len_self)?;
        let label2 = cursor2.get_label(bit_len_other)?;
        let l1 = label1.remaining_bits();
        let l2 = label2.remaining_bits();
        println!("{} {}", l1, l2);
        if l1 > l2 && cursor_self != SliceData::default() && cursor_other != SliceData::default(){
            let time_l = bit_len_self - l1;
            let time2 = bit_len_self - l2;
            println!("{} time", time_l);
            let mut key1 = BuilderData::default();
            let mut key2 = BuilderData::default();
            key1.checked_append_references_and_data(&label1)?;
            key2.checked_append_references_and_data(&label2)?;
            ...
            while queue.len() > ind {
                if queue[ind].2 <= time_l {
                    if queue[ind].1 == key1 {
                        let _res = dict_scan_diff(&mut cursor1, &mut queue[ind].3.clone(), key1.clone(), queue[ind].4.clone(), time_l, queue[ind].5.clone(), &mut op);
                    }
                    else {
                        let _res = dict_scan_diff(&mut SliceData::default(), &mut queue[ind].3.clone(), key1.clone(), queue[ind].4.clone(), time_l, queue[ind].5.clone(), &mut op);
                    }
                }
                else {
                    ...
                    for i in 0..n {
                        ...
                        let mut key1 = queue[ind].1.clone();
                        key1.append_bit_bool(i != 0)?;
                        let ref mut child = SliceData::from(queue[ind].0.clone().reference(i)?);    
        ...
        if l1 < l2 && cursor_self != SliceData::default() && cursor_other != SliceData::default(){
            let time_l = bit_len_self - l2;
            let time1 = bit_len_self - l1;
            let mut key1 = BuilderData::default();
            let mut key2 = BuilderData::default();
            key1.checked_append_references_and_data(&label1)?;
            key2.checked_append_references_and_data(&label2)?;
            ...
            while queue.len() > ind {
                if queue[ind].2 <= time_l {
                    if queue[ind].1 == key2 {
                        let _res = dict_scan_diff(&mut queue[ind].3.clone(), &mut cursor2, queue[ind].4.clone(), key2.clone(), queue[ind].5.clone(), time_l, &mut op);
                    }
                    else {
                        let _res = dict_scan_diff(&mut queue[ind].3.clone(), &mut SliceData::default(), queue[ind].4.clone(), key2.clone(), queue[ind].5.clone(), time_l, &mut op);
                    }
                }
                else {
                    let n = cmp::min(2, queue[ind].0.remaining_references());
        ...
        let _res = dict_scan_diff(&mut cursor_self, &mut cursor_other, BuilderData::default(), BuilderData::default(), bit_len_self, bit_len_other, &mut op);
        Ok(true)
    }
}

fn dict_scan_diff<F>(cursor_1 : &mut SliceData, cursor_2: &mut SliceData, key_buffer_1 : BuilderData, key_buffer_2 : BuilderData, bit_len_1 : usize, bit_len_2 : usize, op: &mut F) -> Result<bool> 
where F: FnMut(SliceData, Option<SliceData>, Option<SliceData>) -> Result<bool> {
    if *cursor_1 == SliceData::default() {
        if  *cursor_2 == SliceData::default() {
            return Ok(true);
        }
        let _res = iterate_internal(
            cursor_2,
            key_buffer_2,
            bit_len_2, &mut |key, cursor| { return op(key.into(), None, Some(cursor.clone())); }
        );
        return Ok(true);
    }
    if *cursor_2 == SliceData::default() {
        ...
        let _res = iterate_internal(
            cursor_1,
            key_buffer_1,
            bit_len_1, &mut |key, cursor| { return op(key.into(), Some(cursor.clone()),  None); }
        );
        return Ok(true);
    }
    if cursor_1.cell().repr_hash() == cursor_2.cell().repr_hash() {
        return Ok(true);  
    }
    ...
    let l1 = label1.remaining_bits();
    let l2 = label2.remaining_bits();
    if bit_len_1 == l1 && bit_len_2 != l2 {
        let mut key1 = key_buffer_1.clone();
        key1.checked_append_references_and_data(&label1)?;
        let mut key2 = key_buffer_2.clone();
        key2.checked_append_references_and_data(&label2)?;
        let fkey : SliceData = key1.into();
        let mut chk = 0;
        let _res = iterate_internal(
            cursor_2,
            key2.clone(),
            bit_len_2 - l2, &mut |key, cursor| { 
                if fkey != key { return op(key.into(), None, Some(cursor.clone())); }
                else { 
                    chk = 1; 
                    if cursor.clone() == cursor_1.clone() {  return Ok(true); }
                    else { return op(key.into(), Some(cursor_1.clone()), Some(cursor.clone())); }
                }
            }
        );
        if chk == 0 { let _res = op(fkey, Some(cursor_1.clone()), None); }
        return Ok(true);
    }
    if bit_len_1 != l1 && bit_len_2 == l2 {
        let mut key2 = key_buffer_2.clone();
        key2.checked_append_references_and_data(&label2)?;
        let fkey : SliceData = key2.into();
        let mut key1 = key_buffer_1.clone();
        key1.checked_append_references_and_data(&label1)?;
        let mut chk = 0;
        let _res = iterate_internal(
            cursor_1,
            key1.clone(),
            bit_len_1 - l1, &mut |key, cursor| { 
                if fkey != key { return op(key.into(), Some(cursor.clone()), None); }
                else { 
                    chk = 1; 
                    if cursor.clone() == cursor_2.clone() {  return Ok(true); }
                    else { return op(key.into(), Some(cursor.clone()), Some(cursor_2.clone())); }
                }
            }
        );
        if chk == 0 { let _res = op(fkey, None, Some(cursor_2.clone())); }
        return Ok(true);
    }
    if bit_len_1 - l1 == bit_len_2 - l2{
        let mut key1 = key_buffer_1.clone();
        ...
        assert!(key1 == key2);
        if bit_len_1 == l1 {
            let _res = op(key1.into(), Some(cursor_1.clone()), Some(cursor_2.clone()));
            return Ok(true);
        }
        let n1 = cmp::min(2, cursor_2.remaining_references());
        let n2 = cmp::min(2, cursor_1.remaining_references());
        let n = cmp::min(n1, n2);
        for i in 0..n {
            let mut key = key1.clone();
            key.append_bit_bool(i != 0)?;
            let ref mut child2 = SliceData::from(cursor_2.reference(i)?);           
            let ref mut child1 = SliceData::from(cursor_1.reference(i)?);
            let _res = dict_scan_diff(child1, child2, key.clone(), key.clone(), bit_len_1 - l1 - 1, bit_len_2 - l2 - 1, op);
        } 
        for i in n..n2 {
            let mut key = key1.clone();
            key.append_bit_bool(i != 0)?;
            let ref mut child = SliceData::from(cursor_1.reference(i)?);
            let _res = dict_scan_diff(child, &mut SliceData::default(), key.clone(), key.clone(), bit_len_1 - l1 - 1, bit_len_2, op);
        }  
        for i in n..n1 {
            let mut key = key1.clone();
            key.append_bit_bool(i != 0)?;
            let ref mut child = SliceData::from(cursor_2.reference(i)?);
            let _res = dict_scan_diff(&mut SliceData::default(), child, key.clone(), key.clone(), bit_len_1, bit_len_2 - l2 - 1, op);
        }   
        return Ok(true);
    }
    return Ok(true);
